...
const getCocktail = (search) => {
  ...
  fetch(apiURL)
    ...
    .then((data) => {
      ...
      for (let i = 1; i < 16; i++) {
        if (
          responseData[`strMeasure${i}`] === null ||
          responseData[`strIngredient${i}`] === null
        ) {
          continue
        } else {
          cocktailData.ingredients.push(
            responseData[`strMeasure${i}`] +
              ' - ' +
              responseData[`strIngredient${i}`]
          )
        }
      }
    ...
    .catch((err) => {
      ...
      const mainMessage =
        'The Cocktail you are looking for is not found. Please check your spelling and try again.'
        ...
        <div class="row g-0">
            ...
            <div class="col-md-8">
                <div class="card-body">
                    ...
                    <p class="card-text badge rounded-pill bg-info text-dark">${
                      JSON.parse(favorite).type
                    }</p><br><br>
                    <button type="button" class="btn btn-secondary btn-sm" value="${
                      JSON.parse(favorite).name
                    }" id="remove-item">Remove Item</button>
                    <button type="button" class="btn btn-secondary btn-sm" value="${
                      JSON.parse(favorite).name
                    }" id="view-item">View Recipe</button>
                </div>
            </div>
        </div>
    </div>`
  ...
  fetch(apiURL)
    ...
    .then((data) => {
      ...
      if (snippet.toLowerCase().includes('cocktail')) {
        ...
        $('.cocktail-description').html(
          ' A cocktail is an alcoholic/non-alcoholic drink consisting of a spirit or spirits mixed with other ingredients, such as fruit juice or cream.'
        )
      }
    })
    .catch((err) => err)
...
const getRecommendations = () => {
  ...
  let ingSel = [favorites.ingOne, favorites.ingTwo][
...
const getCocktailIngredients = (search, recs) => {
  ...
  fetch(apiURL)
    ...
    .then((data) => {
      ...
      let favorites = JSON.parse(localStorage.getItem('favorites'))
      if (
        cocktailData.ingredients.includes(favorites.ingOne) &&
        cocktailData.ingredients.includes(favorites.ingTwo)
      ) {
